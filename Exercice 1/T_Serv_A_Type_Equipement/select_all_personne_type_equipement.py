# select_all_personne_type_equipement.py
...
try:
    ...
    mysql_select_string = ("SELECT ID_Serveur, Nom_Serv , ID_Type_Equipement, Type_Equipement FROM t_serv_a_type_equipement AS T1\n"
                           "INNER JOIN t_type_equipement AS T2 ON T2.ID_Type_Equipement = T1.FK_Type_Equipement\n"
                           "INNER JOIN t_serveur AS T3 ON T3.ID_Serveur = T1.FK_Serveur")
    ...
    for row in records_select:
        print(row['Type_Equipement'],row['Nom_Serv'],)

    for row in records_select:
        output = "Type_Equipement: {Type_Equipement}  Serveur: {Nom_Serv}"
