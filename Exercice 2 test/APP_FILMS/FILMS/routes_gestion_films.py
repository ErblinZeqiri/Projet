...
def films_afficher():
    ...
    if request.method == "GET":
        try:
            ...
            data_serveur = obj_actions_films.films_afficher_data()
            # DEBUG bon marché : Pour afficher un message dans la console.
            print(" data films", data_serveur, "type ", type(data_serveur))
    ...
    return render_template("films/films_afficher.html", data=data_serveur)
...
def films_edit():
    ...
    if request.method == 'GET':
        try:
            # Récupérer la valeur de "id_serveur" du formulaire html "films_afficher.html"
            # l'utilisateur clique sur le lien "edit" et on récupére la valeur de "id_serveur"
            # grâce à la variable "id_serveur_edit_html"
            # <a href="{{ url_for('films_edit', id_serveur_edit_html=row.id_serveur) }}">Edit</a>
            ...
            data_id_serveur = obj_actions_films.edit_films_data(valeur_select_dictionnaire)
            print("dataIdfilms ", data_id_serveur, "type ", type(data_id_serveur))
    ...
    return render_template("films/films_edit.html", data=data_id_serveur)
...
def films_update():
    ...
    if request.method == 'POST':
        try:
            ...
            # Récupérer la valeur de "id_serveur" du formulaire html "films_edit.html"
            # l'utilisateur clique sur le lien "edit" et on récupére la valeur de "id_serveur"
            # grâce à la variable "id_serveur_edit_html"
            # <a href="{{ url_for('film_edit', id_serveur_edit_html=row.id_serveur) }}">Edit</a>
            id_serveur_edit = request.values['id_serveur_edit_html']

            # Récupère le contenu du champ "intitule_films" dans le formulaire HTML "filmsEdit.html"
            nom_serv = request.values['name_edit_intitule_serveur_html']
            valeur_edit_list = [{'id_serveur': id_serveur_edit, 'intitule_serveur': nom_serv}]
            # On ne doit pas accepter des valeurs vides, des valeurs avec des chiffres,
            # des valeurs avec des caractères qui ne sont pas des lettres.
            # Accepte le trait d'union ou l'apostrophe, et l'espace entre deux mots, mais pas plus d'une occurence.
            if not re.match("^([A-Z]|[a-z\u00C0-\u00FF])[A-Za-z\u00C0-\u00FF]*['\\- ]?[A-Za-z\u00C0-\u00FF]+$", nom_serv):
                # En cas d'erreur, conserve la saisie fausse, afin que l'utilisateur constate sa misérable faute
                # Récupère le contenu du champ "intitule_films" dans le formulaire HTML "filmsEdit.html"
                #nom_serv = request.values['name_edit_intitule_films_html']
                ...
                # Exemple d'une liste : [{'id_serveur': 13, 'intitule_films': 'philosophique'}]
                valeur_edit_list = [{'id_serveur': id_serveur_edit, 'intitule_serveur': nom_serv}]
                ...
                # Constitution d'un dictionnaire et insertion dans la BD
                valeur_update_dictionnaire = {"value_id_serveur": id_serveur_edit, "value_name_films": nom_serv}

                # OM 2020.04.09 Objet contenant toutes les méthodes pour gérer (CRUD) les données.
                obj_actions_films = GestionFilms()

                # La commande MySql est envoyée à la BD
                data_id_serveur = obj_actions_films.update_films_data(valeur_update_dictionnaire)
                # DEBUG bon marché :
                print("dataIdfilms ", data_id_serveur, "type ", type(data_id_serveur))
                # Message ci-après permettent de donner un sentiment rassurant aux utilisateurs.
                flash(f"Editer le films d'un films !!!")
...
def films_select_delete():
    ...
    if request.method == 'GET':
        try:
            ...
            # OM 2019.04.02 La commande MySql est envoyée à la BD
            data_id_serveur = obj_actions_films.delete_select_films_data(valeur_delete_dictionnaire)
            ...
            flash(f"Erreur films_delete {erreur.args[0], erreur.args[1]}")
...
def films_delete():
    ...
    if request.method == 'POST':
        try:
            # OM 2020.04.09 Objet contenant toutes les méthodes pour gérer (CRUD) les données.
            obj_actions_films = GestionFilms()
            # OM 2019.04.02 Récupérer la valeur de "id_serveur" du formulaire html "filmsAfficher.html"
            id_serveur_delete = request.form['id_serveur_delete_html']
            # Constitution d'un dictionnaire et insertion dans la BD
            valeur_delete_dictionnaire = {"value_id_serveur": id_serveur_delete}

            data_serveur = obj_actions_films.delete_films_data(valeur_delete_dictionnaire)
