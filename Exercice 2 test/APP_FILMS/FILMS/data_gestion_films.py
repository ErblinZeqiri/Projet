...
class GestionFilms():
    ...
    def films_afficher_data(self):
        try:
            ...
            with MaBaseDeDonnee().connexion_bd.cursor() as mc_afficher:
                ...
                data_serveur = mc_afficher.fetchall()
                # Affichage dans la console
                print("data_serveur ", data_serveur, " Type : ", type(data_serveur))
    ...
    def edit_films_data(self, valeur_id_dictionnaire):
            try:
                ...
                str_sql_id_serveur = "SELECT ID_Serveur, Nom_Serv FROM t_serveur WHERE ID_Serveur = %(value_id_serveur)s"
                ...
                with MaBaseDeDonnee().connexion_bd as mconn_bd:
                    with mconn_bd as mc_cur:
                        mc_cur.mabd_execute(str_sql_id_serveur, valeur_id_dictionnaire)
    ...
    def delete_select_films_data(self, valeur_delete_dictionnaire):
                try:
                    ...
                    with MaBaseDeDonnee().connexion_bd as mconn_bd:
                        with mconn_bd as mc_cur:
                            mc_cur.mabd_execute(str_sql_select_id_serveur, valeur_delete_dictionnaire)
                            data_one = mc_cur.fetchall()
                            print("valeur_id_dictionnaire...", data_one)
                            return data_one
    ...
    def delete_films_data(self, valeur_delete_dictionnaire):
                    try:
                        ...
                        # le "%s" permet d'éviter des injections SQL "simples"
                        # <td><input type = "text" name = "nameEditIntitulefilmsHTML" value="{{ row.intitule_films }}"/></td>
                        #str_sql_delete_intitulefilms = "DELETE FROM `t_serveur` WHERE `t_serveur`.`ID_Serveur` = %(value_id_serveur)s"
                        str_sql_delete_intituleserveur = """DELETE FROM t_serveur WHERE ID_Serveur = %(value_id_serveur)s"""
                        ...
                        with MaBaseDeDonnee().connexion_bd as mconn_bd:
                            with mconn_bd as mc_cur:
                                mc_cur.mabd_execute(str_sql_delete_intituleserveur, valeur_delete_dictionnaire)
                                data_one = mc_cur.fetchall()
                                print("valeur_id_dictionnaire...", data_one)
                                return data_one
                    except (Exception,
