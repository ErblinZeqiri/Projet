...
try:
    ...
    mysql_select_string = ("SELECT ID_Personne, Nom_Pers , ID_Mail, Adresse_Mail FROM  t_pers_a_mail AS T1\n"
    ...
    for row in records_select:
        print(row['Adresse_Mail'],row['Nom_Pers'],)

    for row in records_select:
        output = "Adresse_Mail: {Adresse_Mail}  Personne: {Nom_Pers}"
