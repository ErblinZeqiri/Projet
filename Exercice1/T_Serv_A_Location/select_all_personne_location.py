...
try:
    ...
    mysql_select_string = ("SELECT ID_Serveur, Nom_Serv , ID_Location, Location FROM  t_serv_a_location AS T1\n"
                           "INNER JOIN t_location AS T2 ON T2.ID_Location = T1.FK_Location\n"
    ...
    for row in records_select:
        print(row['Location'],row['Nom_Serv'],)

    for row in records_select:
        output = "Location: {Location}  Serveur: {Nom_Serv}"
