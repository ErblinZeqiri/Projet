...
# OM 2020.04.06 Gestions des "routes" FLASK pour les status.
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /status_afficher
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /status_add ,
...
def status_add ():
    ...
    if request.method == "POST":
        try:
            # OM 2020.04.09 Objet contenant toutes les méthodes pour gérer (CRUD) les données.
            obj_actions_status = GestionStatus()
            # OM 2020.04.09 Récupère le contenu du champ dans le formulaire HTML "status_add.html"
            ...
            if not re.match("^([A-Z]|[a-zÀ-ÖØ-öø-ÿ])[A-Za-zÀ-ÖØ-öø-ÿ]*['\- ]?[A-Za-zÀ-ÖØ-öø-ÿ]+$",
                ...
                # On doit afficher à nouveau le formulaire "status_add.html" à cause des erreurs de "claviotage"
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /status_edit ,
# cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /status_update , cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
# On change la valeur d'un status de status par la commande MySql "UPDATE"
# ---------------------------------------------------------------------------------------------------
    ...
    if request.method == 'POST':
        try:
            ...
            if not re.match("^([A-Z]|[a-zÀ-ÖØ-öø-ÿ])[A-Za-zÀ-ÖØ-öø-ÿ]*['\- ]?[A-Za-zÀ-ÖØ-öø-ÿ]+$",
                ...
                # Récupère le contenu du champ "intitule_status" dans le formulaire HTML "statusEdit.html"
                # name_status = request.values['name_edit_intitule_status_html']
                # Message humiliant à l'attention de l'utilisateur.
                flash(f"Une entrée...incorrecte !! Pas de chiffres, de caractères spéciaux, d'espace à double, "
                      f"de double apostrophe, de double trait union et ne doit pas être vide.", "danger")

                # On doit afficher à nouveau le formulaire "status_edit.html" à cause des erreurs de "claviotage"
                # Constitution d'une liste pour que le formulaire d'édition "status_edit.html" affiche à nouveau
                # la possibilité de modifier l'entrée
                # Exemple d'une liste : [{'ID_Status': 13, 'Status': 'philosophique'}]
                valeur_edit_list = [{'ID_Status': ID_Status_edit,'Status': Status}]

                # DEBUG bon marché :
                # Pour afficher le contenu et le type de valeurs passées au formulaire "status_edit.html"
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /status_select_delete , cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
# On change la valeur d'un status de status par la commande MySql "UPDATE"
# ---------------------------------------------------------------------------------------------------
