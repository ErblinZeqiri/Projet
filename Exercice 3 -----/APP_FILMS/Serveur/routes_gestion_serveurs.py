...
# OM 2020.04.06 Gestions des "routes" FLASK pour les serveurs.
import re
...
from APP_FILMS.Serveur.data_gestion_serveurs import GestionServeurs, MaBdErreurConnexion


# OM 2020.04.16 Afficher les serveurs
    ...
    if request.method == "GET":
        try:
            ...
            if data_serveurs:
                ...
                flash("Données serveurs affichées !!", "success")
            else:
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /serveurs_add ,
...
def serveurs_add ():
    ...
    if request.method == "POST":
        try:
            # OM 2020.04.09 Objet contenant toutes les méthodes pour gérer (CRUD) les données.
            obj_actions_serveurs = GestionServeurs()
            # OM 2020.04.09 Récupère le contenu du champ dans le formulaire HTML "serveurs_add.html"
            ...
            if not re.match("^([A-Z]|[a-zÀ-ÖØ-öø-ÿ])[A-Za-zÀ-ÖØ-öø-ÿ]*['\- ]?[A-Za-zÀ-ÖØ-öø-ÿ]+$",
                ...
                # On doit afficher à nouveau le formulaire "serveurs_add.html" à cause des erreurs de "claviotage"
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /serveurs_edit ,
# cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
...
def serveurs_edit ():
    ...
    if request.method == 'GET':
        try:
            ...
            flash(f"Editer le serveur d'un serveur !!!", "success")
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /serveurs_update , cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
# On change la valeur d'un serveur de serveurs par la commande MySql "UPDATE"
# ---------------------------------------------------------------------------------------------------
    ...
    if request.method == 'POST':
        try:
            ...
            if not re.match("^([A-Z]|[a-zÀ-ÖØ-öø-ÿ])[A-Za-zÀ-ÖØ-öø-ÿ]*['\- ]?[A-Za-zÀ-ÖØ-öø-ÿ]+$",
                ...
                # On doit afficher à nouveau le formulaire "serveurs_edit.html" à cause des erreurs de "claviotage"
                # Constitution d'une liste pour que le formulaire d'édition "serveurs_edit.html" affiche à nouveau
                # la possibilité de modifier l'entrée
                # Exemple d'une liste : [{'id_serveur': 13, 'intitule_serveur': 'philosophique'}]
                valeur_edit_list = [{'ID_Serveur': id_serveur_edit,
                                     'Nom_Serv': Nom_Serv,
                                     'Nombre_Port': Nombre_Port,
                                     'Nombre_U': Nombre_U,
                ...
                # DEBUG bon marché :
                # Pour afficher le contenu et le type de valeurs passées au formulaire "serveurs_edit.html"
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /serveurs_select_delete , cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
# On change la valeur d'un serveur de serveurs par la commande MySql "UPDATE"
# ---------------------------------------------------------------------------------------------------
