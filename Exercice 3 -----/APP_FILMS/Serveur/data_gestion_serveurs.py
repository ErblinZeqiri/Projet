...
class GestionServeurs():
    ...
    def update_serveur_data(self, valeur_update_dictionnaire):
                try:
                    ...
                    # OM 2019.04.02 Commande MySql pour la MODIFICATION de la valeur "CLAVIOTTEE" dans le champ "nameEditIntituleserveurHTML" du form HTML "serveursEdit.html"
                    # le "%s" permet d'éviter des injections SQL "simples"
                    # <td><input type = "text" name = "nameEditIntituleserveurHTML" value="{{ row.intitule_serveur }}"/></td>
                    str_sql_update_intituleserveur = "UPDATE t_serveur SET Nom_Serv = %(value_Nom_Serv)s, Nombre_Port = %(value_Nombre_Port)s," \
                    ...
                    with MaBaseDeDonnee().connexion_bd as mconn_bd:
                        with mconn_bd as mc_cur:
                            mc_cur.execute(str_sql_update_intituleserveur, valeur_update_dictionnaire)
    ...
    def delete_serveur_data(self, valeur_delete_dictionnaire):
                        try:
                            ...
                            with MaBaseDeDonnee().connexion_bd as mconn_bd:
                                with mconn_bd as mc_cur:
                                    mc_cur.execute(str_sql_delete_intituleserveur, valeur_delete_dictionnaire)
                                    data_one = mc_cur.fetchall()
                                    print("valeur_id_dictionnaire...", data_one)
                                    return data_one
                        except (Exception,
