...
# OM 2020.04.06 Gestions des "routes" FLASK pour les Locations.
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /locations_afficher
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /locations_add ,
...
def locations_add ():
    ...
    if request.method == "POST":
        try:
            # OM 2020.04.09 Objet contenant toutes les méthodes pour gérer (CRUD) les données.
            obj_actions_locations = GestionLocations()
            # OM 2020.04.09 Récupère le contenu du champ dans le formulaire HTML "locations_add.html"
            ...
            if not re.match("^([A-Z]|[a-zÀ-ÖØ-öø-ÿ])[A-Za-zÀ-ÖØ-öø-ÿ]*['\- ]?[A-Za-zÀ-ÖØ-öø-ÿ]+$",
                ...
                # On doit afficher à nouveau le formulaire "locations_add.html" à cause des erreurs de "claviotage"
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /locations_edit ,
# cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /locations_update , cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
# On change la valeur d'un location de serveurs par la commande MySql "UPDATE"
# ---------------------------------------------------------------------------------------------------
    ...
    if request.method == 'POST':
        try:
            ...
            if not re.match("^([A-Z]|[a-zÀ-ÖØ-öø-ÿ])[A-Za-zÀ-ÖØ-öø-ÿ]*['\- ]?[A-Za-zÀ-ÖØ-öø-ÿ]+$",
                ...
                # Récupère le contenu du champ "intitule_location" dans le formulaire HTML "locationsEdit.html"
                # name_location = request.values['name_edit_intitule_location_html']
                # Message humiliant à l'attention de l'utilisateur.
                flash(f"Une entrée...incorrecte !! Pas de chiffres, de caractères spéciaux, d'espace à double, "
                      f"de double apostrophe, de double trait union et ne doit pas être vide.", "danger")

                # On doit afficher à nouveau le formulaire "locations_edit.html" à cause des erreurs de "claviotage"
                # Constitution d'une liste pour que le formulaire d'édition "locations_edit.html" affiche à nouveau
                # la possibilité de modifier l'entrée
                # Exemple d'une liste : [{'ID_Location': 13, 'intitule_location': 'philosophique'}]
                valeur_edit_list = [{'ID_Location': ID_Location_edit,
                                     'Location': Location}]

                # DEBUG bon marché :
                # Pour afficher le contenu et le type de valeurs passées au formulaire "locations_edit.html"
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /locations_select_delete , cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
# On change la valeur d'un location de serveurs par la commande MySql "UPDATE"
# ---------------------------------------------------------------------------------------------------
